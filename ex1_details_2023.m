...
Path2EEGLAB = fullfile('C:\Users\Helena\Documents\Helena\Uni\6_Jahr\WiSe22_23\Forschungspraktikum','eeglab2022.1');
% Folder where the function for APICE are:
Path2APICE = fullfile('C:\Users\Helena\Documents\Helena\Uni\6_Jahr\WiSe22_23\Forschungspraktikum','APICE');
% Current path:
Path0 = 'C:\Users\Helena\Documents\Helena\Uni\6_Jahr\WiSe22_23\Forschungspraktikum\APICE\examples\example_original';
Path1 = 'C:\Users\Helena\Documents\Helena\Uni\6_Jahr\WiSe22_23\Forschungspraktikum\MOMA-audio\preprocessed_apice';
% Folder where the scripts defining the parameters are:
Path2Parameters = fullfile(Path0,'parameters');
% Channels location file
filechanloc = fullfile(Path0,'DATA','ElectrodesLayout','standard_1005.elc');
% Folder where the data imported to EEGLAB is: 
Path2DataSet = fullfile(Path1,'DATA','set');
% Folder where the continuos preprocess data will be saved
Path2DataPrp = fullfile(Path1,'DATA','prp');
% Folder where the ERPs will be saved
Path2DataERP = fullfile(Path1,'DATA','erp');
...
%Ppp.eventrmv = {'S1X','S2X','S3X','S4X','S5X','S6X','S7X','S8X','S9X','S10X','S11X','S12X','boundary'};
...
% JEWEILS EXAMPLE ZU EX1 GEÄNDERT -> VORHER NICHT FUNKTIONIERT
...
Perp.Epoch.tw = [-0.200, 0.800];  

% Events relative to which the data epoched 
Perp.Epoch.ev = {'S1','S2','S3','S4','S5','S6','S7','S8','S9','S10','S11','S12'};  
...
%Perp.factors = {'happy' 'fearful'};
%RAUSGENOMMEN GEÄNDERT
...
%-------------------------------------------------------------------------
%SELBST EINGEFÜGT (GEÄNDERT) : REMOVE VEOG
%------------------------------------------------------------------------
...
for sbj=1:length(files2pp)
    ...
    % ---------------------------------------------------------------------
    % Define bad epochs
    [EEG BE] = eega_tDefBEbaddata(EEG, Perp.DefBEa, 'keeppre', 0, 'plot', 0);
    %GEÄNDERT: BE zusätzlich abgespeichert
    %GEÄNDERT
    %EEG.event(BE) = [];
    ...
    % ---------------------------------------------------------------------
    % Reference average
    %GEÄNDERT: if-Schleife selbst hinzugefügt
